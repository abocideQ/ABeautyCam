...
/** Detect five facial landmarks, i.e., two eye centers, nose tip and two mouth corners.
  ...
  */
void CCFAN::FacialPointLocate(const unsigned char *gray_im, int im_width, int im_height,
                              seeta::FaceInfo face_loc, float *facial_loc) {
    ...
    int extend_ly = std::max(int(floor(left_y - (extend_factor - extend_revised_y) * bbox_h)),
                             int(0));
    int extend_ry = std::min(int(floor(right_y + (extend_factor + extend_revised_y) * bbox_h)),
                             int(im_height - 1));
...
/** Extract shape indexed SIFT features.
  ...
  */
void CCFAN::TtSift(const unsigned char *gray_im, int im_width, int im_height, float *face_shape,
                   int patch_size, double *sift_fea) {
    ...
    for (int i = 0; i < pts_num_; i++) {
        /*Get one image patch*/
        GetSubImg(gray_im, im_width, im_height, face_shape[i * 2], face_shape[i * 2 + 1],
                  patch_size, sub_img);
...
/** Extract a image patch which is centered at point(point_x, point_y) with a given patch size.
  ...
  */
void CCFAN::GetSubImg(const unsigned char *gray_im, int im_width, int im_height, float point_x,
                      float point_y, int patch_size, BYTE *sub_img) {
...
bool CCFAN::ResizeImage(const unsigned char *src_im, int src_width, int src_height,
    ...
    for (int n_y_d = 0; n_y_d < dst_height; n_y_d++) {
        for (int n_x_d = 0; n_x_d < dst_width; n_x_d++) {
            ...
            double lf_new_gray =
                    (1 - lf_weight_y) * ((1 - lf_weight_x) * src_im[n_y_s * src_width + n_x_s] +
